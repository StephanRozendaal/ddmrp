...
class StockWarehouseOrderpoint(models.Model):
    ...
    projected_execution_history_chart = fields.Text(
        string='Execution Historical Chart',
        compute='_compute_projected_execution_history_chart',
    )
    ...
    def _compute_projected_execution_history_chart(self):
        start_stack = 0
        ...
        for rec in self:
            ...
            for i in range(int(rec.dlt)):
                moves_today = moves.filtered(lambda m: m.date_expected ==
                                             fields.date.today() + timedelta(
                                                days=i))
                supply_forecast.append(sum(moves_today.mapped('product_qty')))
            ...
            oh_forecast = []
            oh_forecast.append(rec.on_hand_position + supply_forecast[day] +
                               rec.adu)
            for day in supply_forecast-1:
                new_oh = oh_forecast[day-1] + supply_forecast[day] - rec.adu
                oh_forecast.append(new_oh)

            data['on_hand_position'] = [oh_forecast[r] for r in oh_forecast]
            ...
            script, div = components(p)
            rec.projected_execution_history_chart = '%s%s' % (div, script)
